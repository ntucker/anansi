...
describe('makeConfig', () => {
  ...
  describe('basic configuration', () => {
    ...
    it('should use custom rootPath and basePath', () => {
      ...
      const entryArray = Array.isArray(config.entry.App) ? config.entry.App : [config.entry.App];
      expect(entryArray.some(entry => entry && entry.includes('./source'))).toBe(true);
    });
  ...
  describe('module rules', () => {
    it('should have babel loader for TS/JS files', () => {
      ...
      const tsRule = config.module.rules.find(rule =>
        rule.test && rule.test.toString().includes('(t|j)sx')
      );
    ...
    it('should have worker loader', () => {
      ...
      const workerRule = config.module.rules.find(
        rule => rule.oneOf && rule.oneOf.some(subRule =>
